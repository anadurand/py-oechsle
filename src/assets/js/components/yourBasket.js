const  Yourbasket = () => {
  ...
  $('#imgSelected').children('div');
  const total = $('.precio');

  total.reduce(function(valorAnterior, valorActual){
    return valorAnterior + valorActual;
  });

  const rowc2 = $("<div class='row'><div class='col-xs-12 col-sm-12 container-flex-row'></div></div>");
  const btnbuy = $(`button type='button' name='button' class='col-xs-12 col-sm-12 fitRoom--button-class pd'>Buy<span class='precio'>${total}</span></button>`);
